...
@test "give the identifier of the current user" {
    ...
    [[ ${status} -eq 0 ]]
    [[ "$(echo ${lines[0]} | tr -d '\r\n')" = "xuser:x:${uid}:" ]]
    [[ "$(echo ${lines[1]} | tr -d '\r\n')" = "xuser:x:${uid}:${uid}:identity to handle permissions with docker's volumes:/home/xuser:/bin/sh" ]]
}
...
@test "add a group id of 2000" {
    ...
    [[ ${status} -eq 0 ]]
    [[ "$(echo ${lines[0]} | tr -d '\r\n')" = "xuser:x:${gid}:xuser" ]]
    [[ "$(echo ${lines[1]} | tr -d '\r\n')" = "xuser:x:${uid}:${gid}:identity to handle permissions with docker's volumes:/home/xuser:/bin/sh" ]]
}
...
@test "add dgricci as user name" {
    ...
    [[ ${status} -eq 0 ]]
    [[ "$(echo ${lines[0]} | tr -d '\r\n')" = "${unm}:x:${gid}:${unm}" ]]
    [[ "$(echo ${lines[1]} | tr -d '\r\n')" = "${unm}:x:${uid}:${gid}:identity to handle permissions with docker's volumes:/home/${unm}:/bin/sh" ]]
}
